...
export type FishBehavior='cruise'|'skittish'|'dart'|'flee'|'eel';
...
export function verticalOffset(fish:Fish,timeMs:number,phase:number){
  ...
  const amplitude=11+fish.difficulty*2,period=Math.max(900,1900-fish.difficulty*150);
  return Math.sin(timeMs/period*Math.PI*2+phase)*amplitude;
}
